...
inefficient, error-prone, and cumbersome @sql-errors. This is primarily due to way #gls("sql") as a #gls("dsl") is represented in general purpose programming languages and how it is passed to the database. Representing queries as a plain string and passing it to a database directly is dangerous because it can lead to #gls("sql") injection attacks @sqlinject. More sophisticated approaches like #gls("orm")s require users to state classes for every table in a database schema and produce an excessive amount of boilerplate code to link tables and generate methods for all possible queries  @ormpaper. Both approaches do not detect incorrect queries such as invalid type inference from text to integer at compile-time. The error in this query ```sql SELECT * FROM myTable WHERE "1" < 3``` for instance will only be detected after it is already sent to the database in existing #gls("orm") libraries. Another challenge with #gls("sql") embeddings in most general purpose programming languages is the limited support for injecting custom syntax. Java for instance only uses strings or query-like syntax to mimic the #gls("dsl") e.g.: ```Java findUserByStatusAndName(...)``` instead of ```sql SELECT * FROM User WHERE User.status = ... AND User.name = ...``` @ormpaper.

Currently, there is no support for #gls("sql") query generation and validation in Lean 4 with respect to the database schema used. We want to drive the forefront of Lean's general-purpose programming with a strong link to Lean's type system and proof assistant capabilities. Our goal is to express, validate, and execute a query in the PostgreSQL dialect.

The below example query represents a valid query that returns all a names aliased as `nickname` from a customer table where the condition matches, i.e. the id of a customer equals one and the field `birthdate` is before january first 2000. This query is valid under the assumption that the database schema we operate on contains a table named `customer` with the fields `id`, `name`, and `birthdate` of the types `integer`, `varchar 255`, and `date`. Our library should accept such a query (using the original #gls("sql") query syntax) and provide a formal proof that this query is correct according to our embedding.
...
The second example below represents an invalid query that is inconsistent with our embedding regarding  the same schema as for the first example. The fields we selected below (`mame` and `birthday`) are misspelled and thus do not occur in the customer table in our schema and the comparison of the `name` (which has type `varchar 255` in our schema) is invalid for a right hand side of type `date`. Such mistakes should be rejected by our implementation and marked as invalid so that they cannot be sent to a database until they are corrected and a proof of correctness was generated. In order to give users more information about the reason and location of an incorrect query we want to mark the syntax red and provide meaningful error messages upon hover over the marked location. 
...
#todo[Link between check function and type and context]
...
+ `check` provides a proof of the representation
+ Query is fully elaborated
+ Elaborated query type checks and can be used in the library and sent to a database

The following subsections reveal our approach on typing a query, explain the #gls("ast") we designed, and introduce the a type system that we can reason about.

== Typing an SQL Query <Typing>

We must define a type T and context $Gamma$ for queries as basis for the type system we present. Before we define the type of an entire query however, we have to break it down into its `SELECT`, `FROM`, and `WHERE` clauses representing the according parts in a query (```sql SELECT ... FROM ... WHERE ...```). Those sub-parts of a query depend on each other and shape the type we expect an entire query to have. Our approach on typing follows the related literature closely @paper4 @pq @pqgh @psql.

We typed expressions of a `WHERE` clause in the context of the `FROM` clause because it can reference fields from within the `FROM` clause. An expression ```sql WHERE customer.id = 1``` requires the information about whether a customer table is in the scope of a query, has a field `id`, and that the `id`s type is something that compares with `1`. We will continue referring to this type as relation type because it describes the signature of a relation in a relational database. Expressions in Postgres are really flexible #todo[Duplicate with section in Design type system (CAST)] so that even expressions of type `boolean` can be transformed back to numbers and used in further comparisons or arithmetics. So giving an expression the type `boolean` is not sufficient. Instead the type of an expression must be an arbitrary data type.
...
The official Postgres documentation @pq yields a basic structure of the abstract syntax for the Postgres #gls("sql") dialect. We designed the Lean data types closely aligned to the documentation, omitting some features (see @Introduction). @primitive, @operators, and @query-structure represent the #gls("ast") used in this library as #gls("adt")s. All types supported in this library shown in @primitive follow the PSQL specification @pq[pp. 148-221]. The official Postgres documentation separates operators only by boolean and non-boolean operators @pq[pp. 221-244]. To allow for more efficient pattern matching and simpler code, we decided to group the operators further into the categories unary, arithmetic, and comparison shown in @operators.
...
A `SELECT` clause may either mirror the current context (relation type of its `FROM` clause) in which case it is well-formed if T matches $Gamma$ or consist of references of fields in the `FROM` clause. The term (`<context> ⊢ <term> : <type>`) in the #smallcaps[Col] and #smallcaps[Alias] rules refers to a `SelectField` constructor in our #gls("ast") and the term in the #smallcaps[List] and #smallcaps[All] rules to a `Select` constructor. The boolean b of the #smallcaps[All] rule in @select can be chosen arbitrarily between `True` and `False` in our embedding as it limits the results of a query and not its type (see ```sql DISTINCT``` in @AST).
...
All rules for a well-formed `FROM` clause in @from operate directly on the database schema $Gamma$ and build the basis for the `SELECT` and `WHERE` clause of a query. The terms (`<context> ⊢ <term> : <type>`) in the `From` rules (if not explicitly annotated like `Expr e`) refer to a `From` constructor in the #gls("ast"). The simplest rule is table which merely requires the table to exist in a list. The rules `ALIAS`, `JOIN`, `IMPLICITJOIN`, and `NESTEDJOIN` require a previous well-formed `FROM` clauses and therefore always extend/modify the relation type of a table that is defined in the current database schema. The `NESTEDJOIN` rule repeats the requirements for a `SELECT`, `FROM`, and `WHERE` clause instead of leveraging the `QUERY` rule in @query. We made this design choice to avoid cyclic dependencies between the definitions of the `From` and `Query` types and thus, work around mutual inductive types in our implementation. In essence, mutual inductive types are supported but they currently have some restrictions. 
...
In addition to designing and implementing the deep embedding of the fraction of PostgreSQL, we also implemented an interface to interact with actual Postgres database instances in Lean. There are two feasible ways of connecting to databases in Lean 4. The first option is by using a socket library to send TCP packages to the database following the Postgres protocol. This is currently very limited though because cryptographic libraries are still immature in Lean 4. The other option is to use a #gls("ffi"). Following the #gls("ffi") approach we were able to interface with the C library `libPQ` and translate between Lean and C objects. This allowed us to leverage more of the essential features of Postgres e.g. password encryption using scram-sha-256 or advanced options in prepared statements.

= Related Work

#gls("sql") is considered the most widely used database description language for relational databases @sql-widely-used and has been the base for many different dialects. Most programming libraries and papers that introduce a formal embedding of query languages focus on the general #gls("sql") language and cover the basic operations that work across all dialects @chai @paper1 @paper3. Other papers focus on formalizing relational algebra @paper2 @paper4 and provide a translation to #gls("sql"). While those libraries do not support the same features, they offered valuable core ideas on typing queries in relation to a database schema @paper2 @paper1 @paper3 in @Design.

@ex1 depends on a RelationType and carries the proof h with a so-called auto param which acts as a default value (in this case tactic) that proves the proposition. Note that even other arguments such as the expression have to be dependently typed. Those arguments are a substantial part of their types and will remain during computation putting extra weight on the entire #gls("ast"). @ex2 does not store extra arguments on the #gls("ast") type (`From`) but comes with an inductive predicate of the form `Schema → From → RelationType → Prop`. Propositions live in Sort 0 in the Lean type system and are thus stripped off after type checking. This means that the schema overhead is removed during run time and unlike the first approach, will not require additional memory. With a hierarchy of inductive predicates, proofs can be more complicated than just applying the tactics ```lean simp``` or ```lean decide```. This is where a type checking and inference function for each predicate helps to reduce the complexity of proofs over entire queries. Such a type checker can easily be invoked during elaboration and saves tedious manual type inference as part of the elaborator.

Delegating type inference and checking from the elaborator to a separate function causes separation of the error location and syntax. To attach error messages to the correct location in the elaborated syntax such as "```sql SELECT``` #underline(stroke: (paint: red, thickness: 1pt, dash: "densely-dashed"), offset: 2pt, [```sql employee.id```],) ```sql FROM employee ``` ```sql AS b, customer ```", we attached the parsed Lean Syntax reference to the #gls("ast") resulting in an extra `Syntax` field per constructor. We retrieved this information again from the #gls("ast") upon type checking errors.
#example("dependently typed From")[
  ```lean
